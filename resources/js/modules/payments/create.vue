        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div :class="`inline-block w-full align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full duration-200  ${opened ? 'scale-100' : 'scale-0'}`">
                <div class="bg-gray-50 px-4 py-2 border-b border-gray-300 text-right">
                    <h3 class="font-bold text-lg text-gray-700">إضافة دفعة</h3>
                </div>
                <div class="bg-white px-4 pt-5 sm:p-6">
                    <div class="grid grid-cols-2 gap-6">
                        <div class="">
                            ...
                            <small v-if="errors && errors.patient_id" class="text-rose-600 text-xs text-right block">{{ errors.patient_id[0] }}</small>
                        </div>
                        <div class="">
                            <label for="date" class="block text-sm font-medium text-gray-700 text-right">تاريخ الدفعة</label>
                            ...
                            <small v-if="errors && errors.date" class="text-rose-600 text-xs text-right block">{{ errors.date[0] }}</small>
                        </div>
                        ...
                        <div class="">
                            ...
                            <small v-if="errors && errors.amount" class="text-rose-600 text-xs text-right block">{{ errors.amount[0] }}</small>
                        </div>
                        ...
                        <div class="">
                            ...
                            <small v-if="errors && errors.remaining_amount" class="text-rose-600 text-xs text-right block">{{ errors.remaining_amount[0] }}</small>
                        </div>
                        ...
                        <div class="col-span-full">
                            ...
                            <small v-if="errors && errors.notes" class="text-rose-600 text-xs text-right block">{{ errors.notes[0] }}</small>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-revers">
                    ...
                    <CAsyncButton
                        type="button"
                        :loading="submitted"
                        class="w-full inline-flex justify-center rounded-md border border-transparent transition duration-75 transition-all shadow-sm px-4 py-2 bg-sky-700 text-base font-medium text-white hover:bg-sky-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 sm:ml-3 sm:w-auto sm:text-sm"
                        @click="create()"
                    >
                        تأكيد
                    </CAsyncButton>
