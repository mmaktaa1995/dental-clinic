                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:mr-4 sm:text-right">
                            <h3 id="modal-title" class="text-lg leading-6 font-medium text-gray-700">حذف {{ type }}</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">هل أنت متأكد من حذف هذه ال{{ type }}؟</p>
                            </div>
                        </div>
                    </div>
                </div>
...
const deleteItem = async () => {
    ...
    try {
        const response = await axios.delete(`/api/expenses/${id.value}`)
