...
function getRandomColors(colors, count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const colorName = colorKeys[randomIndex]
        randomColors.push({ name: colorName, rgb: colors[colorName], bg: `rgba(${colors[colorName].join(",")}, 0.2)`, border: `rgba(${colors[colorName].join(",")}, .8)` })
    }
            ...
            datasets: [
                {
                    ...
                    backgroundColor: [...randomColors].map((color) => color.bg),
                    borderColor: [...randomColors].map((color) => color.border),
                    // ...color,
                    borderWidth: 1,
                    barThickness: "flex",
                    barPercentage: 0.4,
