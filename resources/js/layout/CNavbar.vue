<template>
    <nav v-if="user && !$route.path.includes('unauthorized') && !$route.path.includes('404')" class="bg-gray-800">
                    ...
                    <div class="mr-3 relative">
                        ...
                        <transition name="fade" duration="150" mode="out-in">
                            <div v-if="show" class="origin-top-left absolute left-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-20" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                                ...
                                <a id="user-menu-item-0" href="javascript:void(0);" class="block px-4 py-2 text-sm text-gray-700 transition transition-all duration-200 bg-white hover:bg-gray-100" role="menuitem" tabindex="-1">مرحبا @{{ user.full_name }}</a>
