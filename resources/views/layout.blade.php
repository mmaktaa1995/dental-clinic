            ...
            <div class="flex flex-col h-0 flex-1">
                ...
                <div class="flex-1 flex flex-col overflow-y-auto bg-gray-800">
                    ...
                    <nav class="px-2 py-4 bg-gray-800" v-if="user && user.admin">
                        ...
                        >
                            <icon-users size="6" class="mr-3 text-gray-400 group-hover:text-gray-300 group-focus:text-gray-300 transition ease-in-out duration-150"> </icon-users>
                            Students
                        </router-link>
                        <router-link
                        ...
                        <router-link
                            ...
                            class="mt-1 group flex items-center px-2 py-2 text-sm leading-5 font-medium text-gray-300 rounded-md hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition ease-in-out duration-150"
                        >
                            <icon-information-circle size="6" class="mr-3 text-gray-400 group-hover:text-gray-300 group-focus:text-gray-300 transition ease-in-out duration-150"> </icon-information-circle>
                            Enrollments
                        </router-link>
                    </nav>
                    <nav class="px-2 py-4 bg-gray-800" v-if="user && !user.admin">
                ...
                <div class="relative flex items-center justify-between h-16">
                    <div class="absolute flex inset-y-0 items-center pr-2 right-0 sm:ml-6 sm:pr-0">
                        ...
                        <div class="ml-3 relative">
                            <div>
                                <button @click="toggleDropdown()" type="button" class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                                    <span class="sr-only">Open user menu</span>
                                    <img class="h-8 w-8 rounded-full" src="https://laracasts.com/images/reviews/tyler-walker.jpg" alt="">
                                </button>
                            </div>
                          ...
                          <transition name="fade" duration="250" mode="out-in">
                              <div v-if="show" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-20" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                                  <!-- Active: "bg-gray-100", Not Active: "" -->
                                  <a href="javascript:void(0);" class="block px-4 py-2 text-sm text-gray-700 transition transition-all duration-200 bg-white hover:bg-gray-100" role="menuitem" tabindex="-1" id="user-menu-item-0">Hello @{{user.full_name}}</a>
                                  <a href="#" @click.prevent="logout()" class="block px-4 py-2 text-sm text-gray-700 transition transition-all duration-200 bg-white hover:bg-gray-100" role="menuitem" tabindex="-1" id="user-menu-item-1">Sign out</a>
                              </div>
                          </transition>
                        </div>
                    </div>
